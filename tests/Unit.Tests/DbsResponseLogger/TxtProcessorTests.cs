...
public class TxtProcessorTests(ITestOutputHelper testOutputHelper)
{
    ...
    public async Task TestDbsTxtResultsFile()
    {
        ...
        await WatchFile(
            monitor,
            data,
            () => UpdateFileModifiedDate(_dir, "txt_process_file0002.txt", -1)
        ); // yesterday
        await WatchFile(
            monitor,
            data,
            () => UpdateFileModifiedDate(_dir, "txt_process_file0003.txt", -2)
        ); // day before yesterday
        await WatchFile(
            monitor,
            data,
            () => UpdateFileModifiedDate(_dir, "txt_process_file0004.txt", -365)
        ); // last year
        ...
        var jsonLogMessages = logMessages
            .Select(msg =>
            {
        ...
        Assert.Equal(
            4,
            jsonLogMessages.Count(x =>
                x["Message"]
                    .Contains(
                        $"[MATCH_COMPLETED] MatchStatus: Match, AgeGroup: {GetAgeRange(testData[1])}, Gender: {GetGender(testData[1])}, Postcode: {GetPostCode(testData[1])}"
                    )
            )
        );
        Assert.Equal(
            ...
            jsonLogMessages.Count(x =>
                x["Message"]
                    .Contains(
                        "The DBS results file has 2 records, batch search resulted in Match='1' and NoMatch='1'"
                    )
            )
        );
    ...
    private static void AssertMatchLogWithTimeStampOfDayExists(
        int expectedCount,
        List<Dictionary<string, string>> jsonLogMessages,
        int relativeDay
    )
    {
        Assert.Equal(
    ...
    private static async Task WatchFile(
        TxtFileMonitor monitor,
        List<string[]> data,
        Func<Task> action
    )
    {
    ...
    private static string GetAgeRange(TestData recordData) =>
        TxtFileProcessor.GetAgeGroup(
            ToDateOnly(recordData.Record[(int)RecordColumn.BirthDate])!.Value
        );
    ...
    private ServiceProvider Bootstrap(List<string> logMessages)
    {
        ...
        servicesCollection.AddLogging(b =>
            b.AddDebug().AddProvider(new JsonTestContextLoggerProvider(TestContext, logMessages))
        );
    ...
    private static async Task WriteTxtAsync(
        TempDirectoryFixture dir,
        string fileName,
        List<string[]> records
    )
    {
        await using var writer = new StreamWriter(
            Path.Combine(dir.IncomingDirectoryPath, fileName)
        );
    ...
    private static Task UpdateFileModifiedDate(
        TempDirectoryFixture dir,
        string fileName,
        int addDays
    )
