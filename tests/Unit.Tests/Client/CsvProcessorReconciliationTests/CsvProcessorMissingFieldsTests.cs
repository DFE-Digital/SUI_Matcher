...
public class CsvProcessorMissingFieldsTests(ITestOutputHelper testOutputHelper) : CsvProcessorTestBase(testOutputHelper)
{
    ...
    public async Task Reconciliation_CsvHeaderDifferences_ShouldShowDifferences()
    {
        ...
        await ReconciliationCsvFileProcessor.WriteCsvAsync(Path.Combine(_tempDir.IncomingDirectoryPath, "file00001.csv"), headers, list);
        ...
        Assert.Equal("NhsNumber - Given - Family", records[0][ReconciliationCsvFileProcessor.HeaderDifferences]);
    }
    ...
    // Test case where there is a hyphen in a name
    [Fact]
    public async Task Reconciliation_CsvHeaderDifferences_ShouldNotShowDifferences_WhenThereAreHyphensInNames()
    {
        var demographicResult = new DemographicResult
        {
            Result = new NhsPerson
        ...
        _matchingService.Setup(x => x.SearchAsync(It.IsAny<SearchSpecification>(), false)).ReturnsAsync(
            new PersonMatchResponse
            {
                Result = new MatchResult { MatchStatus = MatchStatus.Match, NhsNumber = "9449305552" }
            });
        _nhsFhirClient.Setup(x => x.PerformSearchByNhsId(It.IsAny<string>()))
        ...
        var data = new D
        {
            ...
            [TestDataHeaders.GivenName] = "John",
            [TestDataHeaders.Surname] = "Smithy",
            [TestDataHeaders.DOB] = "2000-04-01",
            [TestDataHeaders.PostCode] = "ab12 3ed",
            [TestDataHeaders.Email] = "test@test.com",
            [TestDataHeaders.Phone] = "0789 1234567"
        ...
        await ReconciliationCsvFileProcessor.WriteCsvAsync(
            Path.Combine(_tempDir.IncomingDirectoryPath, "file00001.csv"), headers, list);
        ...
        _nhsFhirClient.Verify(x => x.PerformSearchByNhsId(It.IsAny<string>()), Times.Once,
            "The PerformSearchByNhsId method should have been invoked once");
        (_, List<D> records) =
            await ReconciliationCsvFileProcessor.ReadCsvAsync(monitor.GetLastOperation().AssertSuccess().OutputCsvFile);
