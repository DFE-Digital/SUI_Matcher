...
public class CsvProcessorTests(ITestOutputHelper testOutputHelper)
{
    ...
    public async Task TestCsvBatchFile()
    {
        ...
        var testData = new List<TestData>
        {
            new(
                new D
                {
                    ...
                    [TestDataHeaders.DOB] = f
                        .Date.BetweenDateOnly(new DateOnly(1990, 1, 1), new DateOnly(2020, 1, 1))
        ...
        foreach (var testDataItem in testData)
        {
            _nhsFhirClient
                .Setup(x =>
                    x.PerformSearch(
                        It.Is<SearchQuery>(y => y.Email == testDataItem.Data[TestDataHeaders.Email])
                    )
                )
                .Returns(() => Task.FromResult<SearchResult?>(testDataItem.SearchResult));
        }
        ...
        await ReconciliationCsvFileProcessor.WriteCsvAsync(
            Path.Combine(_dir.IncomingDirectoryPath, "file00002.csv"),
            headers,
            data
        );
    ...
    public async Task TestOneFileSingleMatch()
    {
        ...
        await ReconciliationCsvFileProcessor.WriteCsvAsync(
            Path.Combine(_dir.IncomingDirectoryPath, "file00001.csv"),
            headers,
            list
        );
        ...
        Assert.Equal(
            searchResult.Score.ToString(),
            records[0][MatchingCsvFileProcessor.HeaderScore]
        );
    ...
    public async Task CsvFile_ShouldContainLowConfidenceMatch()
    {
        ...
        await ReconciliationCsvFileProcessor.WriteCsvAsync(
            Path.Combine(_dir.IncomingDirectoryPath, "file00005.csv"),
            headers,
            list
        );
        ...
        Assert.Equal(
            nameof(MatchStatus.LowConfidenceMatch),
            records[0][MatchingCsvFileProcessor.HeaderStatus]
        );
    }
    ...
    public async Task StatsFile_ShouldContainsLowConfidenceStats()
    {
        ...
        Assert.True(
            stats.ContainsKey("CountLowConfidenceMatch"),
            "Stats file should contains LowConfidenceMatches key"
        );
        Assert.True(
            stats.ContainsKey("LowConfidenceMatchPercentage"),
            "Stats file should contains LowConfidenceMatches key"
        );
    ...
    public async Task TestOneFileSingleMatch_GenderIsConverted()
    {
        ...
        var headers = new HashSet<string>(data.Keys);
        await ReconciliationCsvFileProcessor.WriteCsvAsync(
            Path.Combine(_dir.IncomingDirectoryPath, "file00004.csv"),
            headers,
            list
        );
    ...
    {
        var searchResultBad = new SearchResult
        {
            NhsNumber = "AAAAA1111111",
            Score = 0.55m,
            Type = SearchResult.ResultType.Unmatched,
        };
        var searchResultBad2 = new SearchResult
        {
            NhsNumber = "AAAAA1111111",
            Score = 0.55m,
            Type = SearchResult.ResultType.Unmatched,
        };
        var searchResultGood = new SearchResult
        {
            NhsNumber = "AAAAA1111111",
            Score = 0.99m,
            Type = SearchResult.ResultType.Matched,
        ...
        _nhsFhirClient.Verify(
            x => x.PerformSearch(It.Is<SearchQuery>(sq => sq.Gender == null)),
            Times.AtLeast(3),
    ...
    public async Task FileProcessor_ThrowsException_IfFileStaysLocked()
    {
        ...
        _nhsFhirClient
            .Setup(x =>
                x.PerformSearch(
                    It.Is<SearchQuery>(y => y.Email == testData.Data[TestDataHeaders.Email])
                )
            )
            .Returns(() => Task.FromResult<SearchResult?>(testData.SearchResult));
    ...
    public async Task FileProcessor_Processes_AfterFileIsUnlocked()
    {
        ...
        // Simulate file being locked by another process
        await using (
            var stream = File.Open(filePath, FileMode.Open, FileAccess.ReadWrite, FileShare.None)
        )
        {
    ...
    public async Task Reconciliation_ContainsAllHeaders()
    {
        ...
        var addressHistoryFormatted = CsvUtils.WrapInputForCsv(
            demographicResult.Result.AddressHistory
        );
        ...
        Assert.Equal(
            demographicResult.Result.NhsNumber,
            records[0][ReconciliationCsvFileProcessor.HeaderNhsNo]
        );
        Assert.Contains(
            records[0][ReconciliationCsvFileProcessor.HeaderGivenName],
            demographicResult.Result.GivenNames
        );
        Assert.Contains(
            records[0][ReconciliationCsvFileProcessor.HeaderFamilyName],
            demographicResult.Result.FamilyNames
        );
        Assert.Equal(
            demographicResult.Result.BirthDate.ToString(),
            records[0][ReconciliationCsvFileProcessor.HeaderBirthDate]
        );
        Assert.Equal(
            demographicResult.Result.Gender,
            records[0][ReconciliationCsvFileProcessor.HeaderGender]
        );
        Assert.Contains(
            records[0][ReconciliationCsvFileProcessor.HeaderAddressPostalCode],
            demographicResult.Result.AddressPostalCodes
        );
        Assert.Contains(
            records[0][ReconciliationCsvFileProcessor.HeaderEmail],
            demographicResult.Result.Emails
        );
        Assert.Contains(
            records[0][ReconciliationCsvFileProcessor.HeaderPhone],
            demographicResult.Result.PhoneNumbers
        );
        Assert.Equal(
            nameof(ReconciliationStatus.NoDifferences),
            records[0][ReconciliationCsvFileProcessor.HeaderStatus]
        );
        Assert.Contains(String.Empty, records[0][ReconciliationCsvFileProcessor.HeaderDifferences]);
        Assert.Equal(
            addressHistoryFormatted,
            records[0][ReconciliationCsvFileProcessor.HeaderAddressHistory]
        );
        Assert.Equal(
            demographicResult.Result.GeneralPractitionerOdsId,
            records[0][ReconciliationCsvFileProcessor.HeaderGeneralPractitionerOdsId]
        );
        Assert.Equal(
            personMatchResponse.Result.Score.ToString(),
            records[0][ReconciliationCsvFileProcessor.HeaderMatchScore]
        );
        Assert.Equal(
            personMatchResponse.Result.ProcessStage,
            records[0][ReconciliationCsvFileProcessor.HeaderMatchProcessStage]
        );
    }
    ...
    public async Task Reconciliation_OneDifference()
    {
        ...
        Assert.Equal(
            nameof(ReconciliationStatus.Differences),
            records[0][ReconciliationCsvFileProcessor.HeaderStatus]
        );
        Assert.DoesNotContain(
            records[0][TestDataHeaders.GivenName],
            demographicResult.Result.GivenNames
        );
    ...
    public async Task Reconciliation_SupersededNhsNumber()
    {
        ...
        Assert.Equal(
            nameof(ReconciliationStatus.LocalNhsNumberIsSuperseded),
            records[0][ReconciliationCsvFileProcessor.HeaderStatus]
        );
        Assert.Contains(
    ...
    public async Task Reconciliation_MissingNhsNumber()
    {
        ...
        Assert.Equal(
            nameof(ReconciliationStatus.LocalDemographicsDidNotMatchToAnNhsNumber),
            records[0][ReconciliationCsvFileProcessor.HeaderStatus]
        );
    }
    ...
    public async Task Reconciliation_InvalidNhsNumber()
    {
        ...
        Assert.Equal(
            nameof(ReconciliationStatus.LocalNhsNumberIsNotValid),
            records[0][ReconciliationCsvFileProcessor.HeaderStatus]
        );
    }
    ...
    public async Task Reconciliation_ResponseIsNull()
    {
        ...
        // Act
        var result = await reconciliationCsvFileProcessor.ProcessCsvFileAsync(
            filePath,
            _dir.ProcessedDirectoryPath
        );
        ...
        var fieldValuesAddedFromProcessing = records[0]
            .Where(k =>
                k.Key.StartsWith("SUI_")
                && !k.Key.EndsWith("AddressSame")
                && // Field that is always populated.
                !k.Key.EndsWith("Intersect")
                && // Field that is always populated.
                !k.Key.EndsWith("History")
            ) // Field that is always populated.
