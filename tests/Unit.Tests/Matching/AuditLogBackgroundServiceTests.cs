...
public class AuditLogBackgroundServiceTests
{
    ...
    public async Task ExecuteAsync_ShouldWriteAuditLogEntry()
    {
        ...
        _defaultHttpContext.User = new ClaimsPrincipal(
            new ClaimsIdentity([new Claim(ClaimTypes.Name, "TestUser")])
        );
        ...
        mockTableServiceClient
            .Setup(x =>
                x.CreateTableIfNotExistsAsync(It.IsAny<string>(), It.IsAny<CancellationToken>())
            )
            .ReturnsAsync(Mock.Of<Response<TableItem>>());
        mockTableServiceClient
            .Setup(x => x.GetTableClient(SharedConstants.AuditLog.AzStorageTableName))
        ...
        var entry = new AuditLogEntry(
            AuditLogEntry.AuditLogAction.Match,
            new Dictionary<string, string>() { { "SearchId", "12345" } }
        );
        ...
        mockTableServiceClient.Verify(
            x =>
                x.CreateTableIfNotExistsAsync(
                    SharedConstants.AuditLog.AzStorageTableName,
                    It.IsAny<CancellationToken>()
                ),
            Times.Once
        );
    ...
    {
        // Arrange
        _defaultHttpContext.User = new ClaimsPrincipal(new ClaimsIdentity([]));
        var mockTableClient = new Mock<TableClient>();
        mockTableClient
            .Setup(x => x.AddEntityAsync(It.IsAny<TableEntity>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Mock.Of<Response>());
        ...
        _logger.Verify(
            x =>
                x.Log(
                    ...
                    It.Is<It.IsAnyType>(
                        (v, t) =>
                            v.ToString()!.Contains("Audit log entry created with unknown user.")
                    ),
                    null,
