...
public class ReconciliationTests(AppHostFixture fixture, TempDirectoryFixture tempDirectoryFixture)
        ...
        IClassFixture<TempDirectoryFixture>
{
    ...
    {
        await TestAsync(
            "patient_low_confidence_match_9876543210.csv",
            x =>
            {
    ...
    private async Task TestAsync(
        ...
        Action<Dictionary<string, int>> statsAssertions
    )
    {
        ...
        var fileProcessor = new ReconciliationCsvFileProcessor(
            logger,
            mappingConfig,
        ...
        File.Copy(
            ...
            Path.Combine(watcherConfig.Value.IncomingDirectory, inputFileName)
        );
        ...
        var readStatsJson = await File.ReadAllTextAsync(
            monitor.LastOperation.AssertSuccess().StatsJsonFile,
            cts.Token
        );
