...
) : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        ...
        var matchPerson = app.MapPost(
            "/matchperson",
            async (SearchSpecification? model) =>
            {
                if (model is null)
                {
        ...
        var matchPersonNoLogic = app.MapPost(
            "/matchpersonnologic",
            async (PersonSpecificationForNoLogic? model) =>
            {
                if (model is null)
                {
                    return Results.BadRequest(
        ...
        var demographics = app.MapGet(
            "/demographics",
            async ([AsParameters] DemographicRequest request) =>
            {
        ...
        var reconciliation = app.MapPost(
            "/reconciliation",
            async (ReconciliationRequest? model) =>
            {
                if (model is null)
                {
                    return Results.BadRequest(
                ...
                return result.Status == ReconciliationStatus.Error
                    ? Results.BadRequest(result)
                    : Results.Ok(result);
            }
        );
