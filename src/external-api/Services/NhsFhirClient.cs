        ...
        try
        {
            ...
            switch (patient.Entry.Count)
            {
                ...
                case 1:
                    ...
                    var score = patient.Entry[0].Search?.Score;
                    logger.LogInformation("Nhs patient record confidence score {Score}", score);
        ...
        try
        {
            ...
            return new DemographicResult
            {
                Result = new NhsPerson
                {
                    ...
                    FullAddressDetails = data.Address.Select(s => string.Join(" ",s.Line) + " " + s.PostalCode).ToArray(),
