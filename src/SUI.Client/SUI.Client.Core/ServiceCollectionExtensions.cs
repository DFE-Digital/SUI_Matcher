...
public static class ServiceCollectionExtensions
{
    public static IServiceCollection AddClientCore(
        ...
        bool enableReconciliationMode
    )
    {
        services.AddLogging(builder =>
        {
            builder
                .AddConsole(options => options.FormatterName = Shared.SharedConstants.LogFormatter)
                .AddConsoleFormatter<LogConsoleFormatter, ConsoleFormatterOptions>();
            builder.AddProvider(
                new JsonFileLoggerProvider(
                    Path.Combine(
                        Directory.GetCurrentDirectory(),
                        enableReconciliationMode
                            ? "sui-reconciliation-logs.json"
                            : "sui-client-logs.json"
                    )
                )
            );
        });
